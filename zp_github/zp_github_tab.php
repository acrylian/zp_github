...
<body>
   ...
   if (isset($_POST['githubuser'])) {
     ...
     if (isset($_POST['pages-for-repos']) || isset($_POST['articles-for-releases'])) {
       ...
       if (isset($_POST['pages-for-repos'])) {
         ...
         $gh->createRepoPages($markdown, $update);
   ...
   ?> 
				...
				<p><?php echo gettext("Check the option you wish to create items for. Note that the content is served from cache if you have called it before."); ?></p>
    <p><?php echo gettext("If you want fresh contents be sure to clear the cache first, but mind the access limits."); ?></p>
    <form name="create-repo-items" action="zp_github_tab.php" method="post">
      ...
      <fieldset class="githubfieldset">
        ...
        <p><label for="convert-markdown"><input type="checkbox" name="convert-markdown" id="convert-markdown" value="convert-markdown"><?php echo gettext('If checked the README.md file (it must exists, mind the uppercase) of the repo is used and its markdown converted to HTML, otherwise the short description.'); ?></label></p>       
        <p><label for="update-pages"><input type="checkbox" name="update-pages" id="update-pages" value="update-pages"><?php echo gettext('If checked existing pages are updated (overwritten), otherwise only new ones are created.'); ?></label></p> 
      </fieldset>
      <fieldset class="githubfieldset">
        ...
        <p><label for="update-articles"><input type="checkbox" name="update-articles" id="update-articles" value="update-articles"><?php echo gettext('If checked existing articles are updated (overwritten), otherwise only new ones are created.'); ?></label></p> 
